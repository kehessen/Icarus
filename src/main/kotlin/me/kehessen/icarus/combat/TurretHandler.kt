...
private const val TURRET_ARROW_TAG = "TurretArrow"
private const val DEFAULT_WORLD = "world"
private const val INITIAL_AMMO = 0
...
private val PARTICLE_TYPE = Particle.FLAME
...
class TurretHandler(private val plugin: JavaPlugin, config: FileConfiguration, private val menu: MenuHandler) :
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        when (args[0]) {
            "removeall" -> {
                ...
                Bukkit.getWorld(DEFAULT_WORLD)!!.entities.forEach { entity ->
    ...
    private fun reachCheck() {
        if (Bukkit.getOnlinePlayers().none { it.isGliding }) return
        ...
        activeTurrets.forEach outer@{ turret ->
            ...
            if (turretCorrectlyLockedOn(turret, shooter, isShooting)) return@outer

            val reach = turretReach.toDouble()
            turret.getNearbyEntities(reach, reach, reach)
                .forEach { player ->
...
//                    keeping it for now in case something breaks
//                    else {
...
//                    }
    ...
    private fun turretCorrectlyLockedOn(
        turret: ArmorStand,
        shooter: HashMap<ArmorStand, Player>,
        isShooting: MutableSet<ArmorStand>
    ): Boolean {
        if (lockedOn.containsKey(turret)) {
            val player = lockedOn[turret]!!
            if (player.isGliding && player.isOnline && turret.hasLineOfSight(player) &&
                turret.location.distanceSquared(player.location) < turretReach * turretReach
            ) {
                isShooting.add(turret)
                shooter[turret] = player
    ...
    // updates turret ammo and disables empty ones
    private fun updateTurretAmmo() {
        // doing ammo calculation here because it's faster than writing to NBT every single shot

        val toBeDisabled = mutableSetOf<ArmorStand>()
        ...
        if (toBeDisabled.isNotEmpty()) {
            toBeDisabled.forEach { turret ->
        ...
        turrets.forEach { turret ->
            keyNames.forEach inner@{ keyName ->
                ...
                if (turret.persistentDataContainer.has(key)) {
                    ...
                    "ammo" -> turret.persistentDataContainer.set(key, PersistentDataType.INTEGER, INITIAL_AMMO)
                    "active" -> {
    ...
    private fun onRightClick(event: PlayerInteractAtEntityEvent) {
        ...
        menu.createItem(
            AMMO_DISPLAY_MATERIAL, inv, 13, "Ammo", "§r§7Turret ammo: $ammo", "§r§7Add ammo by clicking on arrows"
        )
        menu.createItem(
            SHOT_DELAY_MATERIAL,
            inv,
